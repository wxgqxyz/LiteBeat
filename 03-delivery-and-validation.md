...
- 本文列出的源码、脚本与测试中，T01–T07 的部分已存在于仓库（勾选项即已实测通过），未勾选项仍是计划；不要按“皆未实现”来读这份文档。
...
- [x] 选择兼容的稳定版本并冻结锁文件；确认 Windows MSVC 构建工具与 Node.js 可用。首次生成锁文件可使用安装命令，后续必须使用锁文件安装。
- [x] 配置解析对端口 0、负容量、不存在的静态目录给出可读错误；数据目录首次启动可创建，音乐目录不可擅自创建。
- [x] 实现 `/health/live` 与 `/health/ready`，端口默认 `127.0.0.1:8090`，日志省略敏感字段。
- [x] 前端首页真实请求健康接口；生产加载 `web/dist`；API/媒体 404 保持 JSON/媒体错误，不回退应用 HTML。
...
- [x] 为外键、重复路径、歌单条目重复曲目、事务回滚和 FTS 更新一致性写测试。
- [x] 创建核心表与排序索引；事务中维护规范化字段及 FTS，验证 bundled SQLite 实际支持 FTS5/trigram。
- [x] 每个连接应用并读取核对 PRAGMA；准备语句只在拥有连接的线程内使用。
- [x] 建立有界读写队列，队满立即反馈；写命令超过 4 MiB 总排队载荷时拒绝继续入队。
- [x] 为查询配置 progress handler；请求取消后，未开始的只读任务可以丢弃，写任务必须根据事务和幂等状态处理。
- [x] 迁移失败不标记 ready；数据库版本高于当前程序支持范围时拒绝启动并给出恢复指导。
...
- [x] 先验证未登录访问曲目/媒体/歌单均失败；伪造 Origin、缺失 CSRF、过期 Cookie 被拒绝。
...
- [x] 用自生成短音频和已知字节文件建立测试夹具，记录 SHA-256 和媒体参数。
- [x] 实现鉴权、路径验证、单范围传输、HEAD、多范围策略和不支持 If-Range 条件时完整返回。
- [x] 用响应体包装器持有全局与会话流许可；正常结束、读取错误、客户端断开均释放。
- [x] 为媒体配置 30s 无进展超时；普通 API 的总超时不能截断媒体响应。
- [x] 用浏览器试听 MP3 和 AAC/M4A，拖动后确认播放位置正确；先建立基础兼容矩阵。
...
- [ ] 建立缺标签、中文路径、损坏标签、超大图片标签、链接、无权限目录及扫描断开的夹具。（链接与无权限目录夹具在本机 Windows 无法可靠落盘：符号链接跳过由 `walk.rs` 的 `#[cfg(unix)]` 单元测试覆盖，权限错误仅走 `PERMISSION_DENIED` 归类；其余夹具含扫描中断注入均已建成并集成验证。）
- [x] 惰性遍历，按大小与 mtime 判定增量，未变文件不重新解析；提供 force 重读。
- [x] 建立单子进程和有界 JSON 行协议，限制单条输入/输出长度，阻止损坏文件拖垮服务。
- [x] 实现 Linux 子 cgroup、Windows Job Object 资源限制及退出清理；子进程超时 5s 可杀死并重建。（超时杀死重建与崩溃重建已用真实子进程集成验证；Linux cgroup v2 分支需在真实 Linux 主机复验，能力不可用时按 `unavailable` 如实上报。）
- [x] 扫描成功才把未见条目标记不可用；取消/挂载盘掉线时保留原可用状态。
- [x] 进度统计增量更新，错误明细最多 1000 条；服务重启将运行中任务标记 interrupted。
